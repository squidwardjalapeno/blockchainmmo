...
function eatFoodIfAvailable(hobbit) {
    ...
    if (foodIndex !== -1) {
        ...
        console.log(`😋 Hobbit ${hobbit.name} ate ${food.name}! Restored ${restoreAmount} energy.`);
...
function tryHobbitTrade(hobbit, counterX, counterY) {
    ...
    if (hobbit.job === 'Forager' && pmItem) {
        ...
        if (match) {
            pmItem.count--;
            if (pmItem.count <= 0) hobbit.inventory = hobbit.inventory.filter(i => i !== pmItem);
            ...
            if (socket && socket.connected) {
                socket.emit('buyListing', {
                    storeId, listingId: match.id, buyerWallet: playerWallet,
                    paymentItem: createItem(ITEM_TYPES.PLANT_MATTER), isHobbit: true
                });
            }
            return true;
        } else {
            const alreadyListed = storeData.listings.some(l => l.seller === playerWallet && l.offeredItem.seedType === 'plant_matter');
            if (!alreadyListed) {
                pmItem.count--;
                if (pmItem.count <= 0) hobbit.inventory = hobbit.inventory.filter(i => i !== pmItem);
                if (socket && socket.connected) {
                    socket.emit('createListing', {
                        storeId, wallet: playerWallet,
                        offeredItem: createItem(ITEM_TYPES.PLANT_MATTER), wantedType: 'egg'
                    });
                }
                return true;
            }
        ...
        const match = storeData.listings.find(l => l.wantedType === foodItem.seedType && l.offeredItem.seedType === 'plant_matter');
        if (match) {
            foodItem.count--;
            if (foodItem.count <= 0) hobbit.inventory = hobbit.inventory.filter(i => i !== foodItem);

            giveItemToHobbit(hobbit, createItem(ITEM_TYPES.PLANT_MATTER)); 
            if (socket && socket.connected) {
                socket.emit('buyListing', {
                    storeId, listingId: match.id, buyerWallet: playerWallet,
                    paymentItem: createItem(ITEM_TYPES[foodItem.seedType.toUpperCase()]), isHobbit: true
                });
            }
            return true;
        } else {
            const alreadyListed = storeData.listings.some(l => l.seller === playerWallet && l.offeredItem.seedType === foodItem.seedType);
            if (!alreadyListed) {
                foodItem.count--;
                if (foodItem.count <= 0) hobbit.inventory = hobbit.inventory.filter(i => i !== foodItem);
                if (socket && socket.connected) {
                    socket.emit('createListing', {
                        storeId, wallet: playerWallet,
                        offeredItem: createItem(ITEM_TYPES[foodItem.seedType.toUpperCase()]), wantedType: 'plant_matter'
                    });
                }
                return true;
            }
...
function isWalkableForHobbit(tx, ty, worldMatrix, roomMatrix, hobbit = null, fromX = null, fromY = null) {
    ...
    if (fromX !== null && fromY !== null) {
        ...
        if (cRoom !== tRoom) {
            ...
            if (!doors.includes(data.tileID) && !doors.includes(fromData.tileID)) return false; 
        }
    }
    return true;
}
...
// Fast Pointer-Based BFS Pathfinder
function findPathToCoords(startTX, startTY, targetTX, targetTY, worldMatrix, roomMatrix, hobbit = null) {
    ...
    const maxDepth = 80; 
    let head = 0; // Pointer replaces queue.shift() to keep execution GC free
...
function findNearestMaturePlant(hobbit, range = 480) { 
    ...
    for (let [key, plant] of plants) {
        const px = plant.gx * 16 + 8, py = plant.gy * 16 + 8;
        const dist = Math.hypot(px - hobbit.x, py - hobbit.y);
        
        if (dist < range && dist < minDist) {
            if (plant.growth >= 100) {
                minDist = dist; nearest = plant;
            }
        }
    }
    return nearest;
...
export function spawnHobbit(gx, gy, houseId = null, homeX = null, homeY = null, defaultJob = 'Forager') {
    ...
    const keyItem = houseId ? {
        name: `Key to House #${houseId}`, seedType: "key", spriteID: 38, tileset: "keyTileset",
        isKey: true, houseId, baseHealth: 100, count: 1, maxStack: 1
    } : null;

    hobbits.push({
        id: 'hobbit_' + Math.random().toString(36).substr(2, 9),
        name, job: defaultJob, isHobbit: true,
        x: gx * 16, y: gy * 16, floor: 1, speed: 35,
        hp: 40, maxHp: 40, ad: 2, energy: 100, maxEnergy: 100,
        inventory: keyItem ? [keyItem] : [], houseId, homeX, homeY,
        doorX: houseId ? homeX - 1 : null,  
        doorY: houseId ? homeY + 1 : null,  
        chestX: houseId ? homeX - 2 : null, 
        chestY: houseId ? homeY : null,     
        hitboxLeft: 4, hitboxRight: 12, hitboxTop: 10, hitboxBottom: 15,
        state: 'idle', goal: 'wander', dir: 'South', frame: 0, animTimer: 0,
        moveTimer: Math.random() * 3, pathTimer: 0, attackTimer: 0, path: [],
        targetPlant: null, lastUpdated: Date.now(), slowTickTimer: Math.random() * 1.5
    });
}

// ==========================================
// ⚙️ MAIN REAL-TIME AI STATE MACHINE
// ==========================================
export function updateHobbits(modifier, worldMatrix, roomMatrix) {
    ...
    // Dead entity cleaning sweep
    for (let i = hobbits.length - 1; i >= 0; i--) {
        const hob = hobbits[i];
        if (hob.hp <= 0) {
            hob.inventory.forEach(item => {
                import('./bacteria.js').then(m => {
                    m.seedBacteria(Math.floor(hob.x / 16), Math.floor(hob.y / 16), item.seedType, item.isKey ? item.houseId : item.health, item.virulence);
                });
            });
            import('./bacteria.js').then(m => m.seedBacteria(Math.floor(hob.x / 16), Math.floor(hob.y / 16), "raw_chicken", 50, 0));
            hobbits.splice(i, 1);
            continue;
        }
    }
    ...
    hobbits.forEach(hobbit => {
        ...
        if (Math.abs(hobbitCX - heroCX) > 1 || Math.abs(hobbitCY - heroCY) > 1) return;
        ...
        // Catch-up fast-forward simulation
        if (deltaSeconds > 2.0) {
            let timeRemaining = Math.min(deltaSeconds, 86400); 
            let simX = Math.floor(hobbit.x / 16), simY = Math.floor(hobbit.y / 16);
            ...
            while (timeRemaining > 0) {
                ...
                if (hobbit.energy < 30) eatFoodIfAvailable(hobbit);

                const hx = hero.x + 8, hy = hero.y + 8;
                const distToHero = Math.hypot(hx - (simX * 16 + 8), hy - (simY * 16 + 8));

                if (distToHero < 80 && hero.hp > 0) {
                    if (distToHero <= 24) {
                        hero.hp = Math.max(0, hero.hp - hobbit.ad);
                        if (socket && socket.connected) socket.emit('updateStats', { hp: hero.hp });
                    } else {
                        const path = findPathToCoords(simX, simY, Math.floor(hx / 16), Math.floor(hy / 16), worldMatrix, roomMatrix, hobbit);
        ...
        const inViewport = (screenX >= -pad && screenX <= viewport.screen[0] + pad && screenY >= -pad && screenY <= viewport.screen[1] + pad);
        ...
        if (!inViewport) {
            ...
            if (hobbit.slowTickTimer <= 0) {
                ...
                const px = (hero.x + 8) - (hobbit.x + 8), py = (hero.y + 8) - (hobbit.y + 8);
                const distToHero = Math.hypot(px, py);

                if (distToHero < 80 && hero.hp > 0) { target = hero; targetDist = distToHero; }
                ...
                if (hobbit.energy < 30) {
                    ...
                    if (!ate && hobbit.houseId && hobbit.chestX !== null) {
                        ...
                        if (foodIdx !== -1) {
                            ...
                            foodItem.count--;
                            if (foodItem.count <= 0) chestItems.splice(foodIdx, 1);
                            if (socket && socket.connected) socket.emit('updateChest', { chestId, items: chestItems });
                            giveItemToHobbit(hobbit, { ...foodItem, count: 1 });
                ...
                if (!hobbit.path || hobbit.path.length === 0) {
                    if (target && targetDist > 20) {
                        const path = findPathToCoords(currTX, currTY, Math.floor((target.x + 8) / 16), Math.floor((target.y + 8) / 16), worldMatrix, roomMatrix, hobbit);
                        if (path) { hobbit.path = path; hobbit.goal = 'engage'; }
                    } else if (hobbit.job === 'Trader' && hobbit.houseId) {
                        const doorKey = `${hobbit.doorX}_${hobbit.doorY}`;
                        const doorState = doorStates.get(doorKey);
                        const isLocked = doorState ? doorState.locked : true;
                        ...
                        if (!worldTime.isNight) {
                            if (isLocked) {
                                if (distToDoor <= 1) {
                                    if (socket && socket.connected) socket.emit('setDoorLock', { gx: hobbit.doorX, gy: hobbit.doorY, locked: false });
                                } else {
                                    const path = findPathToCoords(currTX, currTY, hobbit.doorX, hobbit.doorY, worldMatrix, roomMatrix, hobbit);
                                    if (path) { hobbit.path = path; hobbit.goal = 'unlock_door'; }
                                }
                            } else {
                            ...
                            if (!isLocked) {
                                if (distToDoor <= 1) {
                                    if (socket && socket.connected) socket.emit('setDoorLock', { gx: hobbit.doorX, gy: hobbit.doorY, locked: true });
                                } else {
                                    const path = findPathToCoords(currTX, currTY, hobbit.doorX, hobbit.doorY, worldMatrix, roomMatrix, hobbit);
                                    if (path) { hobbit.path = path; hobbit.goal = 'lock_door'; }
                                }
                            } else {
                                if (currTX === hobbit.homeX && currTY === hobbit.homeY) { hobbit.goal = 'sleep'; }
                                else {
                                    const path = findPathToCoords(currTX, currTY, hobbit.homeX, hobbit.homeY, worldMatrix, roomMatrix, hobbit);
                                    if (path) { hobbit.path = path; hobbit.goal = 'sleep'; }
                                }
                            }
                        }
                    } else if (worldTime.isNight && hobbit.houseId) {
                        if (currTX !== hobbit.homeX || currTY !== hobbit.homeY) {
                            const path = findPathToCoords(currTX, currTY, hobbit.doorX, hobbit.doorY, worldMatrix, roomMatrix, hobbit);
                            if (path) { hobbit.path = path; hobbit.goal = 'gohome'; }
                        }
                    } else {
                        assignRandomWalk(hobbit, currTX, currTY, worldMatrix, roomMatrix);
                        hobbit.goal = 'wander';
                ...
                if (hobbit.path && hobbit.path.length > 0) {
                    ...
                    hobbit.x = nextNode.x * 16; hobbit.y = nextNode.y * 16;
        ...
        if (target && targetDist <= 20) {
            ...
            if (hobbit.state !== 'attacking') {
                hobbit.state = 'attacking'; hobbit.frame = 0; hobbit.animTimer = 0; hobbit.attackTimer = 0.5; hobbit.path = []; 
                const tdx = target.x - hobbit.x, tdy = target.y - hobbit.y;
            ...
            if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking' && hobbit.pathTimer <= 0) {
                ...
                else { assignRandomWalk(hobbit, currTX, currTY, worldMatrix, roomMatrix); hobbit.goal = 'wander'; hobbit.state = hobbit.path.length > 0 ? 'walking' : 'idle'; }
            }
        } 
            ...
            if (!ate) {
                if (hobbit.houseId && hobbit.chestX !== null) {
                    ...
                    const depositTX = hobbit.chestX + 1, depositTY = hobbit.chestY;

                    if (currTX === depositTX && currTY === depositTY) {
                        hobbit.state = 'idle'; hobbit.path = [];
                        const chestId = `chest_${hobbit.chestX}_${hobbit.chestY}`;
                        if (!chestCache.has(chestId)) {
                            ...
                            if (foodIdx !== -1) {
                                ...
                                if (nearest) { hobbit.goal = 'harvest_food'; hobbit.targetPlant = nearest; }
                                else { assignRandomWalk(hobbit, currTX, currTY, worldMatrix, roomMatrix); hobbit.goal = 'wander'; hobbit.state = hobbit.path.length > 0 ? 'walking' : 'idle'; }
                            }
                        }
                        ...
                        if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking') {
                            hobbit.path = [{ x: hobbit.doorX, y: hobbit.doorY - 1 }, { x: hobbit.homeX, y: hobbit.homeY }];
            ...
            if (hobbit.targetPlant) {
                ...
                if (livePlant && livePlant.growth >= 100) {
                    ...
                    if (dist <= 24) {
                        ...
                        if (keyName && ITEM_TYPES[keyName]) giveItemToHobbit(hobbit, createItem(ITEM_TYPES[keyName]));
                        
                        plants.delete(plantKey);
                        if (socket && socket.connected) socket.emit('syncTile', { gx: livePlant.gx, gy: livePlant.gy, traits: 0 });
                        hobbit.targetPlant = null; hobbit.goal = 'wander'; eatFoodIfAvailable(hobbit); 
                    } else {
                        if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking' && hobbit.pathTimer <= 0) {
                            hobbit.pathTimer = 2.0;
                ...
                } else { hobbit.targetPlant = null; hobbit.goal = 'wander'; }
            } else { hobbit.goal = 'wander'; }
        }
        else if (hobbit.job === 'Trader' && hobbit.houseId) {
            const doorKey = `${hobbit.doorX}_${hobbit.doorY}`;
            const doorState = doorStates.get(doorKey);
            ...
            if (hasEggs) {
                ...
                if (counter) {
                    ...
                    const standX = counter.x, standY = counter.y + 1;
                    ...
                    if (roomID === storeId) {
                        ...
                        if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking' && hobbit.pathTimer <= 0) {
                            hobbit.pathTimer = 2.0;
                            const path = findPathToCoords(currTX, currTY, counter.x - 1, counter.y + 2, worldMatrix, roomMatrix, hobbit);
                            if (path) { hobbit.path = path; hobbit.state = 'walking'; }
                        }
                    }
                }
                    ...
                    if (roomID === hobbit.houseId) {
                        ...
                        if (dist <= 24) {
                            ...
                            if (!hayStorageCache.has(storageId)) {
                                ...
                                const pmItem = hobbit.inventory.find(i => i.seedType === 'plant_matter');
                                if (pmItem) {
                                    const exist = hayItems.find(i => i.seedType === 'plant_matter' && i.count < (i.maxStack || 64));
                                    if (exist) { exist.count += pmItem.count; hobbit.inventory = hobbit.inventory.filter(i => i !== pmItem); }
                                    else if (hayItems.length < 8) { hayItems.push(pmItem); hobbit.inventory = hobbit.inventory.filter(i => i !== pmItem); }
                                }
                                if (socket && socket.connected) socket.emit('updateHayStorage', { hayStorageId: storageId, items: hayItems });
                            }
                        } else {
                            if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking' && hobbit.pathTimer <= 0) {
                                hobbit.pathTimer = 2.0;
                                const path = findPathToCoords(currTX, currTY, storage.x, storage.y, worldMatrix, roomMatrix, hobbit);
                                if (path) { hobbit.path = path; hobbit.state = 'walking'; }
                            }
                        }
                    } else {
                ...
                if (egg) {
                    ...
                    if (dist <= 24) {
                        hobbit.state = 'idle'; hobbit.path = [];
                        giveItemToHobbit(hobbit, createItem(ITEM_TYPES.EGG)); 
                        if (socket && socket.connected) socket.emit('syncTile', { gx: egg.gx, gy: egg.gy, traits: 0 });
                    } else {
                        if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking' && hobbit.pathTimer <= 0) {
                            hobbit.pathTimer = 2.0;
            ...
            if (hobbit.job === 'Forager') {
                ...
                if (isInventoryFull && hasPM && isChestFull) {
                    ...
                    if (currTX === depositTX && currTY === depositTY) {
                        ...
                        if (chestCache.has(chestId)) {
                            ...
                            if (pmIdx !== -1) {
                                ...
                                if (pmItemInChest.count <= 0) chestItems.splice(pmIdx, 1);
                        ...
                        } else if (socket && socket.connected) socket.emit('requestChest', chestId);
                    } else {
                        if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking' && hobbit.pathTimer <= 0) {
                            hobbit.pathTimer = 2.0;
                            const path = findPathToCoords(currTX, currTY, depositTX, depositTY, worldMatrix, roomMatrix, hobbit);
                ...
                else if (isInventoryFull || ((hasOtherLoot || hasPM) && !nearestPlant && !hobbit.targetPlant)) {
                    hobbit.goal = 'deposit';
                    const depositTX = hobbit.chestX + 1, depositTY = hobbit.chestY;
                    ...
                    if (currTX === depositTX && currTY === depositTY) {
                        ...
                        if (chestCache.has(chestId)) {
                            ...
                            depositItems.forEach(dep => {
                                const exist = chestItems.find(i => i.seedType === dep.seedType && i.count < (i.maxStack || 8));
                                if (exist) {
                                    const space = (exist.maxStack || 8) - exist.count;
                                    if (dep.count <= space) { exist.count += dep.count; hobbit.inventory = hobbit.inventory.filter(i => i !== dep); }
                                    else { exist.count = exist.maxStack || 8; dep.count -= space; }
                                } else if (chestItems.length < 8) {
                                    chestItems.push(dep); hobbit.inventory = hobbit.inventory.filter(i => i !== dep);
                                }
                            });
                            if (socket && socket.connected) socket.emit('updateChest', { chestId, items: chestItems });
                        } else if (socket && socket.connected) socket.emit('requestChest', chestId);
                    ...
                    if (roomID !== 0 && roomID !== 9999) {
                        if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking') {
                            const path = findPathToCoords(currTX, currTY, hobbit.doorX, hobbit.doorY - 1, worldMatrix, roomMatrix, hobbit);
                            if (path) { hobbit.path = path; hobbit.state = 'walking'; }
                        }
                    } else {
                        if (hobbit.targetPlant) {
                            ...
                            if (livePlant && livePlant.growth >= 100) {
                                ...
                                if (dist <= 24) {
                                    ...
                                    if (ITEM_TYPES[seedKey] && Math.random() < 0.6) giveItemToHobbit(hobbit, createItem(ITEM_TYPES[seedKey]));

                                    plants.delete(plantKey);
                                    if (socket && socket.connected) socket.emit('syncTile', { gx: livePlant.gx, gy: livePlant.gy, traits: 0 });

                                    hobbit.targetPlant = null; hobbit.path = []; hobbit.state = 'idle';
                                } else {
                                    if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking' && hobbit.pathTimer <= 0) {
                                        hobbit.pathTimer = 2.0;
                                        const path = findPathToCoords(currTX, currTY, livePlant.gx, livePlant.gy, worldMatrix, roomMatrix, hobbit);
                                        if (path) { hobbit.path = path; hobbit.state = 'walking'; }
                                        else { hobbit.targetPlant = null; }
                                    }
                                }
                            } else { hobbit.targetPlant = null; }
                        } else {
                            const nearest = findNearestMaturePlant(hobbit);
                            if (nearest) { hobbit.targetPlant = nearest; }
                            else {
                                hobbit.goal = 'wander';
                                if ((!hobbit.path || hobbit.path.length === 0) && hobbit.state !== 'attacking') {
                                    hobbit.moveTimer -= modifier;
        ...
        // Animation and state translation
        if (hobbit.state === 'attacking') {
            ...
            if (oldTimer > 0.25 && hobbit.attackTimer <= 0.25) {
                ...
                if (dist <= 24 && hero.hp > 0) {
                    hero.hp = Math.max(0, hero.hp - hobbit.ad);
                    if (socket && socket.connected) socket.emit('updateStats', { hp: hero.hp });
                }
            }
            hobbit.frame = 0; 
            if (hobbit.attackTimer <= 0) { hobbit.state = 'idle'; hobbit.moveTimer = 1.0; }
        }
        else if (hobbit.path && hobbit.path.length > 0 && hobbit.state === 'walking') {
            const nextNode = hobbit.path[0];
            const targetX = nextNode.x * 16, targetY = nextNode.y * 16;
            const dx = targetX - hobbit.x, dy = targetY - hobbit.y;
            const dist = Math.sqrt(dx * dx + dy * dy);

            const directions = ['East', 'SouthEast', 'South', 'SouthWest', 'West', 'NorthWest', 'North', 'NorthEast'];
            hobbit.dir = directions[Math.round(8 * Math.atan2(dy, dx) / (2 * Math.PI) + 8) % 8];

            if (dist > 2) {
                moveEntity(hobbit, (dx / dist) * hobbit.speed * modifier, (dy / dist) * hobbit.speed * modifier, worldMatrix, roomMatrix);
            } else {
                hobbit.x = targetX; hobbit.y = targetY; hobbit.path.shift(); 
            }
