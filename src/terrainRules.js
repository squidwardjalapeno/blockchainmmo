...
export function applyShorelineRules(cx, cy, worldMap) {
    ...
    const isLand = (xOff, yOff) => {
        const col = cx + xOff;
        const row = cy + yOff;
        // 🛡️ THE FIX: Use CONFIG.MAP_SIZE instead of 100
        if (col < 0 || col >= CONFIG.MAP_SIZE || row < 0 || row >= CONFIG.MAP_SIZE) return false;
        
        // 🛡️ THE FIX: Multiply row by MAP_SIZE for the 1D Array lookup
        return worldMap[row * CONFIG.MAP_SIZE + col] >= landThresh;
    };

    // Bitmasking (N=1, W=2, E=4, S=8)
    if (isLand(0, -1)) score += 1;   // North neighbor is Land
    if (isLand(-1, 0)) score += 2;   // West neighbor is Land
    if (isLand(1, 0))  score += 4;   // East neighbor is Land
    if (isLand(0, 1))  score += 8;   // South neighbor is Land
    ...
    return { score, config: SHORELINE_CONFIGS[score] || "openWater" };
}
