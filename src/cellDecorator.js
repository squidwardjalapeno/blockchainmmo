...
import { spawnChicken } from './animals.js';
...
// ==========================================
// 🏗️ BUILDING FOUNDATION STAMPER (REDUCES BOILERPLATE)
// ==========================================
export function stampBuildingBase(gx, gy, w, h, currentId, tiles, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const { floor = 42, roofTop = 40, roofMid = 48, wall = 3, wallDepth = 1 } = tiles;

    // 1. Flooring stamp
    for (let i = 0; i < w; i++) {
        for (let j = -(h - 1); j <= 0; j++) {
            setGlobalTile(gx + i, gy + j, floor, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    ...
    // 2. Roof Line Top
    for (let i = 0; i < w; i++) {
        setGlobalTile(gx + i, gy - (h - 1), roofTop, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    }

    // 3. Roof Lines Middle
    const roofMidEnd = -(wallDepth);
    for (let i = 0; i < w; i++) {
        for (let j = -(h - 2); j <= roofMidEnd; j++) {
            setGlobalTile(gx + i, gy + j, roofMid, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
        }
    }

    // 4. Wall Base Lines
    for (let i = 0; i < w; i++) {
        for (let j = -(wallDepth - 1); j <= 0; j++) {
            setGlobalTile(gx + i, gy + j, wall, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
        }
    }
}

// ==========================================
// 🏗️ BLUEPRINT REGISTRY HELPERS
// ==========================================
...
export function setGlobalTile(gx, gy, tileID, roomID, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    if (worldMatrix[cx][cy] === null || !worldMatrix[cx][cy]) {
        const blueprintIdx = (cy * CONFIG.MAP_SIZE) + cx;
        const isLand = worldMap[blueprintIdx] >= CONFIG.LAND_THRESHOLD || worldMap[blueprintIdx] >= 100;
        worldMatrix[cx][cy] = new Uint16Array(10000).fill(isLand ? 63 : 17);
        roomMatrix[cx][cy] = new Uint16Array(10000).fill(0);
...
export function ensureZoneInitialized(cx, cy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    console.log(`🎪 Lazy loading settlement area at Well [${zoneWell.x}, ${zoneWell.y}]`);
...
export function generateWellSpokes(well) {
    ...
    for (let i = 0; i < numSpokes; i++) {
        ...
        for (let r = 0; r < 350; r += 2) { 
            ...
            let py = well.y + dy * r;
            for (let b of boxes) {
                if (px >= b.minX && px <= b.maxX && py >= b.minY && py <= b.maxY) maxR = r; 
            }
        }
        rawRadii[i] = maxR;
    }
...
export function drawHouse(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    stampBuildingBase(gx, gy, 4, 3, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 48, wallDepth: 1 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Door (closed)
    setGlobalTile(gx + 2, gy, 52, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Wall
    
    registerObject(gx, gy - 1, 'CHEST_STORAGE', { houseId: currentId });
...
export function drawTemple(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++; 
    stampBuildingBase(gx, gy, 4, 8, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 1, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Entrance Door
...
export function drawGeneralStore(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    stampBuildingBase(gx, gy, 4, 4, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 3, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Door
...
export function drawVillageHall(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    stampBuildingBase(gx, gy, 11, 9, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 3, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap);

    const midX = 5, midY = -4;
    for (let j = -8; j <= 0; j++) {
        if (j !== -1 && j !== -6) registerObject(gx + midX, gy + j, 'INT_WALL', { houseId: currentId });
    }
    for (let i = 0; i < 11; i++) {
        if (i !== 2 && i !== 8) registerObject(gx + i, gy + midY, 'INT_WALL', { houseId: currentId });
    }
...
export function drawRootCellar(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    stampBuildingBase(gx, gy, 3, 3, currentId, { floor: 42, roofTop: 367, roofMid: 48, wall: 50, wallDepth: 1 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Door
    setGlobalTile(gx, gy - 2, 367, 0, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Walkable top overlay
    setGlobalTile(gx + 1, gy - 2, 367, 0, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 2, gy - 2, 367, 0, worldMatrix, roomMatrix, fertilityMatrix, worldMap);

    registerObject(gx, gy - 1, 'FOOD_STORAGE', { houseId: currentId });
...
export function drawBarn(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    stampBuildingBase(gx, gy, 4, 3, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 5, wallDepth: 1 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
        ...
        for (let j = -(height - 1); j <= 0; j++) {
            ...
            if (isTop || isBottom || isLeft || isRight) {
                ...
                if (seededRandom() > 0.85) {
                    const age = Math.floor(seededRandom() * 100);
                    ...
                    import('./plants.js').then(m => m.createPlant(tx, ty, fertilityMatrix, age, randomCrop));
                }
            }
        }
    }
...
export function drawStorageRoom(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    stampBuildingBase(gx, gy, 5, 5, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 50, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Door
...
export function drawBarracks(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    stampBuildingBase(gx, gy, 6, 6, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 1, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Entrance Door
...
export function drawTwoStoryHouse(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    stampBuildingBase(gx, gy, 4, 4, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 3, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Front door
...
export function drawInn(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    roomMetadata[currentId] = { type: 'TWO_STORY', frontY: gy, maxOffset: -8 };
    stampBuildingBase(gx, gy, 8, 9, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 50, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
...
export function drawMilitaryQuarters(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    roomMetadata[currentId] = { type: 'TWO_STORY', frontY: gy, maxOffset: -10 };
    stampBuildingBase(gx, gy, 10, 11, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 3, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
...
export function drawBlacksmith(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    stampBuildingBase(gx, gy, 6, 5, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 1, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 1, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
...
export function drawForge(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    roomMetadata[currentId] = { type: 'STANDARD', frontY: gy, depth: 5 };
    stampBuildingBase(gx, gy, 5, 5, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 1, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 2, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
...
export function drawWorkshop(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    stampBuildingBase(gx, gy, 6, 6, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 1, wallDepth: 3 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 2, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
...
export function drawLargeBarn(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    roomMetadata[currentId] = { type: 'LARGE_BARN', frontX: gx, frontY: gy };
    stampBuildingBase(gx, gy, 6, 6, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 5, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    
    // Stamp the upper extra T-structure extension for Large Barn manually
    for (let i = 2; i <= 3; i++) {
        for (let j = -7; j <= -6; j++) {
...
export function drawTownHall(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    const currentId = nextHouseId++;
    roomMetadata[currentId] = { type: 'TWO_STORY', frontY: gy, maxOffset: -11 };
    stampBuildingBase(gx, gy, 11, 12, currentId, { floor: 42, roofTop: 40, roofMid: 48, wall: 3, wallDepth: 2 }, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
    setGlobalTile(gx + 5, gy, 49, currentId, worldMatrix, roomMatrix, fertilityMatrix, worldMap); // Door
...
export function planVillage(gvx, gvy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    villageChunks.forEach(c => {
        ...
        zoneLookup.set(key, villageChunks);
        zoneWellLookup.set(key, wellObj);
    });
    ...
    for (let x = -halfW; x < halfW; x++) {
        for (let y = -halfH; y < halfH; y++) {
            ...
            if (isEdgeX || isEdgeY) {
                ...
                if (isGate) setGlobalTile(gx, gy, 6, 0, worldMatrix, roomMatrix, fertilityMatrix, worldMap); 
                else setGlobalTile(gx, gy, 11, 0, worldMatrix, roomMatrix, fertilityMatrix, worldMap); 
            }
        }
    }
}
...
// ==========================================
// 🛣️ ROAD & WALL BRESENHAM PAINTERS
// ==========================================
export function drawInterCellRoad(startX, startY, endX, endY, worldMatrix, roomMatrix, fertilityMatrix, worldMap, tileID = 337, thickness = 3, avoidObstacles = false) {
    ...
    const paintTile = (targetX, targetY) => {
        ...
        if (avoidObstacles) {
            for (let ox = -1; ox <= 1; ox++) {
                for (let oy = -1; oy <= 1; oy++) {
                    const checkX = targetX + ox, checkY = targetY + oy;
                    const cCX = Math.floor(checkX / 100), cCY = Math.floor(checkY / 100);
                    if (worldMatrix[cCX]?.[cCY]) {
                        const cLX = ((checkX % 100) + 100) % 100, cLY = ((checkY % 100) + 100) % 100;
...
export function drawPlannedRanchRoads(worldMatrix, roomMatrix, fertilityMatrix, worldMap, targetWellX = null, targetWellY = null) {
    ...
    plannedRanches.forEach(r => {
        ...
        const gateX = r.gx + r.gateX, gateY = r.gy;
        let startX = gateX, startY = gateY + 2; 
...
export function drawRingRoads(worldMatrix, roomMatrix, fertilityMatrix, worldMap, targetWell = null) {
    ...
    wellsToProcess.forEach(well => {
        ...
        well.spokes.forEach(s => {
            waypoints.push({ x: Math.floor(well.x + s.dx * s.r), y: Math.floor(well.y + s.dy * s.r) });
        });

        for (let i = 0; i < waypoints.length; i++) {
            const p1 = waypoints[i], p2 = waypoints[(i + 1) % waypoints.length];
...
export function drawTownWalls(worldMatrix, roomMatrix, fertilityMatrix, worldMap, targetWell = null) {
    ...
    wellsToProcess.forEach(well => {
            ...
            for(let ox = -1; ox <= 1; ox++) {
                for(let oy = -1; oy <= 1; oy++) {
                    ...
                    if(worldMatrix[nCX]?.[nCY]) {
                        ...
                        if ([337, 208, 12, 13, 6].includes(nTID)) return; 
                    }
                }
            }
        ...
        for (let i = 0; i < wallWaypoints.length; i++) {
            const p1 = wallWaypoints[i], p2 = wallWaypoints[(i + 1) % wallWaypoints.length];
...
export function decorateCell(cx, cy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    if (!ecoGenerated.has(cellKey)) {
            ...
            for (let oy = -1; oy <= 1; oy++) {
                for (let ox = -1; ox <= 1; ox++) {
                    ...
                    if (nx >= 0 && nx < CONFIG.MAP_SIZE && ny >= 0 && ny < CONFIG.MAP_SIZE) {
                        ...
                        if (neighborType === 104 || neighborType < CONFIG.LAND_THRESHOLD) isForestRegion = true;
                    }
                }
            }
        }
        ...
        if (isForestRegion) {
            ...
            const spawnThreshold = [104, 101, 102, 103, 107].includes(cellType) ? 0.55 : 0.80;
                ...
                for (let lx = 0; lx < 100; lx++) { 
                    ...
                    if (worldMatrix[cx][cy][idx] === 63 && roomMatrix[cx][cy][idx] === 0) {
                        if (seededRandom() > spawnThreshold) { 
                            const gx = (cx * 100) + lx, gy = (cy * 100) + ly;
                            if (isInsideVillagePolygon(gx, gy)) continue;
                            
                            let isClear = true;
                            for (let ox = -1; ox <= 1; ox++) {
                                if (getObjectAt(gx + ox, gy)) { isClear = false; break; }
                            }

                            if (isClear) {
                                for (let ox = 0; ox <= 1; ox++) {
                                    const checkGX = gx + ox, checkGY = gy;
                                    const tCX = Math.floor(checkGX / 100), tCY = Math.floor(checkGY / 100);
                                    if (tCX >= 0 && tCX < CONFIG.MAP_SIZE && tCY >= 0 && tCY < CONFIG.MAP_SIZE) {
                                        if (worldMatrix[tCX] && worldMatrix[tCX][tCY]) {
                                            ...
                                            if (tID !== 63 || (rID !== 0 && rID !== 9999)) { isClear = false; break; }
                                        }
                                    } else { isClear = false; break; }
                                }
                            }
                            if (isClear) registerObject(gx, gy, 'FOREST_TREE');
                        }
                    }
                }
            }
        ...
        if (socket && socket.connected) socket.emit('requestChunkPlants', { cx, cy });
    }
}

// ==========================================
// ⭕ RADIAL TREE CULLER (Inside Settlement)
// ==========================================
function isInsideVillagePolygon(gx, gy) {
    for (let i = 0; i < plannedWells.length; i++) {
        ...
        if (well.spokes && well.spokes.length > 0) {
            const dx = gx - well.x, dy = gy - well.y;
    ...
    for (let ly = 0; ly < 100; ly++) {
        for (let lx = 0; lx < 100; lx++) {
            ...
            if (worldMatrix[cx][cy][ly * 100 + lx] === 63 && !isBase(gx, gy)) {
                ...
                else if ([6, 9, 7, 11, 13, 14, 15].includes(mask)) borderTile = fillTileId; 
                else if (mask === 0) {
                ...
                if (borderTile !== null) newTiles.push({ idx: ly * 100 + lx, tile: borderTile });
            }
        }
    }
...
export function linkLakes(worldMap, worldMatrix, roomMatrix, fertilityMatrix) {
    ...
    const centers = lakes.map(lake => {
        let sumX = 0, sumY = 0;
        lake.forEach(idx => { sumX += idx % CONFIG.MAP_SIZE; sumY += Math.floor(idx / CONFIG.MAP_SIZE); });
        return { tx: Math.floor(sumX / lake.length) * 100 + 50, ty: Math.floor(sumY / lake.length) * 100 + 50 };
    });

    for (let i = 0; i < centers.length; i++) {
        const lakeA = centers[i];
        let closest = null, shortest = Infinity;
        for (let j = 0; j < centers.length; j++) {
            if (i === j) continue;
            const dist = Math.abs(lakeA.tx - centers[j].tx) + Math.abs(lakeA.ty - centers[j].ty);
            if (dist < shortest) { shortest = dist; closest = centers[j]; }
        }
        if (closest) {
            let curX = lakeA.tx, curY = lakeA.ty;
            while (curX !== closest.tx || curY !== closest.ty) {
                if (Math.abs(curX - closest.tx) > Math.abs(curY - closest.ty)) {
                    if (curX < closest.tx) curX++; else curX--;
                } else {
                    if (curY < closest.ty) curY++; else curY--;
                }
                for (let ox = -1; ox <= 1; ox++) {
                    for (let oy = -1; oy <= 1; oy++) {
...
// ==========================================
// 🏰 SETTLEMENT LAYOUT OVERWORLD PLANNERS
// ==========================================
export function planAllSettlements(worldMap, worldMatrix, roomMatrix, fertilityMatrix) {
    ...
    const size = CONFIG.MAP_SIZE, seed = window.worldSeed || 1; 

    for (let i = 0; i < worldMap.length; i++) {
        const type = worldMap[i];
        if ([101, 102, 103, 107].includes(type)) {
            ...
            else if (type === 107) planMiningCamp(tx, ty, worldMatrix, roomMatrix, fertilityMatrix, worldMap);
        }
    }
}
...
export function planMiningCamp(gvx, gvy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    while (placedTents < numTents && attempts < 100) {
        attempts++;
        const angle = seededRandom() * Math.PI * 2, dist = 6 + (seededRandom() * 6);
    ...
    while (placedFires < numFires && attempts < 50) {
        attempts++;
        const tx = gvx + Math.floor(seededRandom() * 16) - 8, ty = gvy + Math.floor(seededRandom() * 16) - 8;
...
export function drawCampfireArea(gx, gy, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    const seats = [{ dx: 0, dy: -1 }, { dx: 0, dy: 1 }, { dx: -1, dy: 0 }, { dx: 1, dy: 0 }];
...
export function ensureLocalCells(hero, worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    ...
    for (let ox = -1; ox <= 1; ox++) {
        for (let oy = -1; oy <= 1; oy++) {
            const cx = heroCX + ox, cy = heroCY + oy;
    ...
    for (let cx = 0; cx < CONFIG.MAP_SIZE; cx++) {
        for (let cy = 0; cy < CONFIG.MAP_SIZE; cy++) {
            ...
            for (let idx = 0; idx < 10000; idx++) {
                if (roomMatrix[cx][cy][idx] === 9998) roomMatrix[cx][cy][idx] = 0;
            }
        }
    }
}
...
export function populateWorld(worldMap) {
    ...
    for (let i = 0; i < worldMap.length; i++) {
        if (worldMap[i] >= CONFIG.LAND_THRESHOLD) {
            const roll = seededRandom(); 
            if (roll > 1.9999517) worldMap[i] = 103; 
            else if (roll > 1.99942) worldMap[i] = 102; 
            else if (roll > 0.9978) worldMap[i] = 107; 
            else if (roll > 0.99305) worldMap[i] = 101; 
        }
    }
...
export function linkVillages(worldMap, worldMatrix, roomMatrix, fertilityMatrix) {
    const size = CONFIG.MAP_SIZE, adjacencyList = new Map(), settlements = [], seed = window.worldSeed || 1; 

    for (let i = 0; i < worldMap.length; i++) {
        if ([101, 102, 103].includes(worldMap[i])) {
            const type = worldMap[i], cx = i % size, cy = Math.floor(i / size);
            let tx, ty;
            if (type === 103) { tx = cx * 100 + 100; ty = cy * 100 + 100; } 
            else if (type === 102) { tx = cx * 100 + 50; ty = cy * 100 + 50; } 
            else {
    ...
    for (let i = 0; i < settlements.length; i++) {
        ...
        for (let j = i + 1; j < settlements.length; j++) {
            ...
            if (distSq > 0 && distSq <= maxRangeSq) {
                let redundant = false;
                for (let k = 0; k < settlements.length; k++) {
                    if (k === i || k === j) continue;
                    const C = settlements[k];
                    if (Math.pow(A.x - C.x, 2) + Math.pow(A.y - C.y, 2) < distSq && 
                        Math.pow(B.x - C.x, 2) + Math.pow(B.y - C.y, 2) < distSq) {
                ...
                if (!redundant) {
                    ...
                    drawInterCellRoad(A.tx, A.ty, B.tx, B.ty, worldMatrix, roomMatrix, fertilityMatrix, worldMap, 337, 2);
                }
            }
        }
    }
...
export function buildPlannedStructures(worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    plannedBuildings.forEach(b => { b.func(b.args[0], b.args[1], worldMatrix, roomMatrix, fertilityMatrix, worldMap); });
    plannedRanches.forEach(r => { drawRanch(r.gx, r.gy, r.w, r.h, r.gateX, r.barnType, worldMatrix, roomMatrix, fertilityMatrix, worldMap); });
}

export function buildPlannedWells(worldMatrix, roomMatrix, fertilityMatrix, worldMap) {
    plannedWells.forEach(w => { registerObject(w.x, w.y + 1, 'WELL_OBJECT'); });
}